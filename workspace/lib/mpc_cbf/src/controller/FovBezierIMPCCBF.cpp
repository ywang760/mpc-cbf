...
bool FovBezierIMPCCBF<T, DIM>::optimize(std::vector<SingleParameterPiecewiseCurve>& result_curves,
    ...
    if (slack_mode_) {
        ...
        std::sort(idx.begin(), idx.end(),
                  [this, current_pos, other_pos_covs](size_t a_idx, size_t b_idx) {
                      return compareDist(current_pos, other_pos_covs.at(a_idx),
                                         other_pos_covs.at(b_idx));
                  });
    ...
    for (size_t iter = 0; iter < impc_iter_; ++iter) {
        ...
        for (size_t i = 0; i < other_robot_positions.size(); ++i) {
            ...
            qp_generator_.piecewise_mpc_qp_generator_ptr()->addHyperplaneConstraintForPiece(
                0,
                shifted_hyperplane); // add the collision avoidance constraint on the first segment
        }
        ...
        if (iter == 0) {
            for (size_t i = 0; i < other_robot_positions.size(); ++i) {
                ...
                if (!slack_mode_) {
                    ...
                    qp_generator_.addRangeCBFConstraintWithSlackVariables(current_state, other_xy,
                                                                          i);
                }
            }
            ...
            for (size_t i = 0; i < other_robot_positions.size(); ++i) {
                ...
                for (size_t k = 0; k < cbf_horizon_; ++k) {
                    T distance_to_ellipse =
                        distanceToEllipse(pred_states.at(k).pos_, other_xy, other_xy_cov);
                    slack_values.push_back(0); // TODO pass in param
                }
                //                    qp_generator_.addPredSafetyCBFConstraints(pred_states,
                //                    other_xy);
                if (!slack_mode_) {
                    ...
                    qp_generator_.addPredSafetyCBFConstraintsWithSlackVariables(pred_states,
                                                                                other_xy, i);
                    ...
                    qp_generator_.addPredFovRBConstraintsWithSlackVariables(pred_states, other_xy,
                                                                            i);
                    qp_generator_.addPredRangeCBFConstraintsWithSlackVariables(pred_states,
                                                                               other_xy, i);
                }
            }
        }
...
template <typename T, unsigned int DIM>
T FovBezierIMPCCBF<T, DIM>::distanceToEllipse(const VectorDIM& robot_pos, const Vector& target_mean,
    ...
    if (!isinf(target_cov(0, 0))) {
        ...
        T theta =
            atan2(eigenvectors(1, m),
                  eigenvectors(0, m)); // angle of the major axis wrt positive x-asis (ccw rotation)
        if (theta < 0.0) {
            theta += M_PI;
        } // angle in [0, 2pi]

        T slope = atan2(-target_mean(1) + robot_pos(1), -target_mean(0) + robot_pos(0));
        T x_n = target_mean(0) + a * cos(slope - theta) * cos(theta) -
                b * sin(slope - theta) * sin(theta);
        T y_n = target_mean(1) + a * cos(slope - theta) * sin(theta) +
...
bool FovBezierIMPCCBF<T, DIM>::compareDist(const VectorDIM& p_current,
    ...
    return distanceToEllipse(p_current, a.first, a.second) <
           distanceToEllipse(p_current, b.first, b.second);
}
