...
namespace mpc_cbf {
    ...
    ConnectivityIMPCCBF<T, DIM>::ConnectivityIMPCCBF(Params &p, std::shared_ptr<DoubleIntegrator> model_ptr,
    ...
    collision_shape_ptr_(collision_shape_ptr) {
        ...
        std::unique_ptr<ConnectivityMPCCBFQPOperations> connectivity_mpc_cbf_operations_ptr =
                std::make_unique<ConnectivityMPCCBFQPOperations>(p.mpc_cbf_params, model_ptr, connectivity_cbf_ptr);
        qp_generator_.addPiecewise(std::move(connectivity_mpc_cbf_operations_ptr), num_neighbors, slack_mode_);
        ...
        assert(Ts <= h);
        assert(std::abs(h - static_cast<int>(h/Ts) * Ts) < 1e-10); // Use epsilon for floating point comparison
    ...
    template <typename T, unsigned int DIM>
    bool ConnectivityIMPCCBF<T, DIM>::optimize(std::vector<SingleParameterPiecewiseCurve> &result_curves,
        ...
        const size_t num_neighbors = other_robot_positions.size();
        ...
        if (slack_mode_) {
            ...
            for (size_t i = 0; i < num_neighbors; ++i) {
                ...
                distances[i] = (other_robot_positions[i].head(spatial_dims) - current_pos.head(spatial_dims)).norm();
            }
            
            // Sort by pre-computed distances (closer robots get priority)
            std::sort(idx.begin(), idx.end(), [&distances](size_t a, size_t b) {
        ...
        for (size_t iter = 0; iter < impc_iter_; ++iter) {
            ...
            if (iter == 0) {
                for (size_t i = 0; i < other_robot_positions.size(); ++i) {
                    Vector other_xy(2);
                    other_xy << other_robot_positions[i](0), other_robot_positions[i](1);
                    ...
                    if (!slack_mode_) {
                        ...
                        // qp_generator_.addConnectivityConstraint(current_state, other_xy, slack_value);
                    } else {
                        qp_generator_.addSafetyCBFConstraintWithSlackVariables(current_state, other_xy, i);
                        // FIXME: Implement connectivity-specific constraint methods with slack
                        // qp_generator_.addConnectivityConstraintWithSlackVariables(current_state, other_xy, i);
                    }
                }
                ...
                const std::vector<T> slack_values(cbf_horizon_, 0); // TODO: pass in param
                ...
                for (size_t i = 0; i < other_robot_positions.size(); ++i) {
                    ...
                    if (!slack_mode_) {
                        ...
                        // qp_generator_.addPredConnectivityConstraints(pred_states, other_xy, slack_values);
                    } else {
                        qp_generator_.addPredSafetyCBFConstraintsWithSlackVariables(pred_states, other_xy, i);
                        // FIXME: Implement predicted connectivity constraint methods with slack
                        // qp_generator_.addPredConnectivityConstraintsWithSlackVariables(pred_states, other_xy, i);
                    }
                }
            }
    ...
    template <typename T, unsigned int DIM>
    typename ConnectivityIMPCCBF<T, DIM>::Vector
    ConnectivityIMPCCBF<T, DIM>::generatorDerivativeControlInputs(uint64_t derivative_degree) {
