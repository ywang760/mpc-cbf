...
FovMPCCBFQPOperations<T, DIM>::FovMPCCBFQPOperations(Params& p,
...
FovMPCCBFQPOperations<T, DIM>::safetyCBFConstraint(const State& current_state,
    ...
    A0.segment(0, DIM) = a;
    Row A_control_pts =
        -1.0 * A0.transpose() * this->U_basis_; // [1, num_piece*dim*num_control_pts]
...
FovMPCCBFQPOperations<T, DIM>::fovLBConstraint(const State& current_state, const Vector& other_pos,
                                               T slack_value) {
    Vector state(2 * DIM);
    state << current_state.pos_, current_state.vel_;
    // cbf constraint A,b
    ...
    linear_constraints.push_back(
        LinearConstraint(A_control_pts, std::numeric_limits<T>::lowest(), b + slack_value));

    Vector A1 = Vector::Zero(DIM * this->k_hor_); // [3K, 1]
    A1.segment(DIM, DIM) = a;
    Row A1_control_pts =
        -1.0 * A1.transpose() * this->U_basis_; // [1, num_piece*dim*num_control_pts]
    return linear_constraints;
}

template <typename T, unsigned int DIM>
std::vector<typename FovMPCCBFQPOperations<T, DIM>::LinearConstraint>
FovMPCCBFQPOperations<T, DIM>::fovRBConstraint(const State& current_state, const Vector& other_pos,
                                               T slack_value) {
    Vector state(2 * DIM);
    state << current_state.pos_, current_state.vel_;
    // cbf constraint A,b
...
FovMPCCBFQPOperations<T, DIM>::predSafetyCBFConstraints(const std::vector<State>& pred_states,
    ...
    for (size_t k = 0; k < pred_states.size(); ++k) {
        ...
        Row Ak_control_pts =
            -1.0 * Ak.transpose() * this->U_basis_; // [1, num_piece*dim*num_control_pts]
        linear_constraints.push_back(
