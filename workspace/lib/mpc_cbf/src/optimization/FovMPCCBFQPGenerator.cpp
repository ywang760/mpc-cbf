...
namespace mpc_cbf {
    ...
    void FovMPCCBFQPGenerator<T, DIM>::addPiecewise(
            std::unique_ptr<FovMPCCBFQPOperations> &&piecewise_mpc_cbf_operations_ptr, int num_neighbors, bool slack_mode) {
    ...
    template <typename T, unsigned int DIM>
    void FovMPCCBFQPGenerator<T, DIM>::addPredSafetyCBFConstraints(const std::vector<State> &pred_states,
                                                                               const Vector &other_pos,
    ...
    template <typename T, unsigned int DIM>
    void FovMPCCBFQPGenerator<T, DIM>::addFovLBConstraintWithSlackVariables(
            const State &current_state,
            const Vector &other_pos,
            std::size_t neighbor_idx) {
        ...
        slack_coefficients(neighbor_idx) = -1;
        for (size_t i = 0; i < linear_constraints.size(); ++i) {
            this->addLinearConstraintForPiecewiseWithSlackVariables(linear_constraints.at(i), slack_coefficients);
        }
    }

    template <typename T, unsigned int DIM>
    void FovMPCCBFQPGenerator<T, DIM>::addFovRBConstraintWithSlackVariables(
            const State &current_state,
            const Vector &other_pos,
            std::size_t neighbor_idx) {
