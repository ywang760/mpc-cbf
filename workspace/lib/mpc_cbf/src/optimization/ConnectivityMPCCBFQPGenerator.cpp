...
namespace mpc_cbf {
    ...
    template <typename T, unsigned int DIM>
    void ConnectivityMPCCBFQPGenerator<T, DIM>::addSafetyCBFConstraint(
        const Vector &current_state, const Vector &neighbor_state,
        T slack_value) {
      LinearConstraint linear_constraint =
          piecewise_mpc_cbf_operations_ptr_->safetyCBFConstraint(
    ...
    void ConnectivityMPCCBFQPGenerator<T, DIM>::addConnectivityConstraint(
        ...
        T slack_value) {
      LinearConstraint linear_constraint =
          piecewise_mpc_cbf_operations_ptr_->connectivityConstraint(
              current_state, neighbor_state, slack_value);
      this->piecewise_mpc_qp_generator_ptr_->addLinearConstraintForPiecewise(
          linear_constraint);
    }
    ...
    void ConnectivityMPCCBFQPGenerator<T, DIM>::addPredSafetyCBFConstraints(
      ...
      std::vector<LinearConstraint> linear_constraints =
          piecewise_mpc_cbf_operations_ptr_->predSafetyCBFConstraints(
              pred_states, neighbor_state);
    ...
    void ConnectivityMPCCBFQPGenerator<T, DIM>::addPredConnectivityConstraints(
        ...
        const std::vector<T> &slack_values) {
      std::vector<LinearConstraint> linear_constraints =
          piecewise_mpc_cbf_operations_ptr_->predConnectivityConstraints(
              pred_states, neighbor_state);
      for (size_t i = 0; i < linear_constraints.size(); ++i) {
        this->piecewise_mpc_qp_generator_ptr_->addLinearConstraintForPiecewise(
            linear_constraints.at(i));
    ...
    template <typename T, unsigned int DIM>
    void ConnectivityMPCCBFQPGenerator<T, DIM>::
        addSafetyCBFConstraintWithSlackVariables(const Vector &current_state,
      ...
      this->addLinearConstraintForPiecewiseWithSlackVariables(
          linear_constraint, slack_coefficients);
    }

    template <typename T, unsigned int DIM>
    void ConnectivityMPCCBFQPGenerator<T, DIM>::
        addConnectivityConstraintWithSlackVariables(
            const Vector &current_state, const Vector &neighbor_state,
            std::size_t neighbor_idx) {
      LinearConstraint linear_constraint =
          piecewise_mpc_cbf_operations_ptr_->connectivityConstraint(
    ...
    template <typename T, unsigned int DIM>
    void ConnectivityMPCCBFQPGenerator<T, DIM>::
        addPredSafetyCBFConstraintsWithSlackVariables(
            const std::vector<State> &pred_states, const Vector &neighbor_state,
            std::size_t neighbor_idx) {
      std::vector<LinearConstraint> linear_constraints =
          piecewise_mpc_cbf_operations_ptr_->predSafetyCBFConstraints(
      ...
      for (const auto &constraint : linear_constraints) {
        this->addLinearConstraintForPiecewiseWithSlackVariables(
            constraint, slack_coefficients);
      }
    }

    template <typename T, unsigned int DIM>
    void ConnectivityMPCCBFQPGenerator<T, DIM>::
        addPredConnectivityConstraintsWithSlackVariables(
            const std::vector<State> &pred_states, const Vector &neighbor_state,
            std::size_t neighbor_idx) {
      std::vector<LinearConstraint> linear_constraints =
          piecewise_mpc_cbf_operations_ptr_->predConnectivityConstraints(
