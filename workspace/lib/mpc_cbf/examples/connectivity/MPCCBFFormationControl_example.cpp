...
// FIXME: these two includes should include the ConnectivityCBF
...
int main(int argc, char* argv[]) {
    ...
    // init cbf
    // FIXME: here is changed to ConnectivityCBF
    ...
    // FIXME: the following call needs to be changed: num_neighbors should be size_t (not int)
    ...
    // TODO: check if init_state and current_state are necessary
    ...
    while (sim_t < sim_runtime) {
        ...
        for (int robot_idx = 0; robot_idx < num_robots; ++robot_idx) {
            ...
            // TODO: check if other_robot_positions is necessary
            ...
            // TODO: following could be optimized
            if (!success) {
                ...
                if (!trajs.empty()) {
                    ...
                    pred_traj_ptrs.at(robot_idx) = std::make_shared<SingleParameterPiecewiseCurve>(trajs.back());
