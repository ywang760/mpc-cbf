...
class FovMPCCBFQPOperations : public MPCCBFQPOperationsBase<T, DIM> {
  public:
    ...
    FovMPCCBFQPOperations(Params& p, std::shared_ptr<DoubleIntegrator> model_ptr,
    ...
    LinearConstraint safetyCBFConstraint(const State& current_state, const Vector& other_pos,
                                         T slack_value = 0);
    std::vector<LinearConstraint> fovLBConstraint(const State& current_state,
                                                  const Vector& other_pos, T slack_value = 0);
    std::vector<LinearConstraint> fovRBConstraint(const State& current_state,
                                                  const Vector& other_pos, T slack_value = 0);
    LinearConstraint rangeCBFConstraint(const State& current_state, const Vector& other_pos,
                                        T slack_value = 0);
    ...
    std::vector<LinearConstraint> predFovLBConstraints(const std::vector<State>& pred_states,
                                                       const Vector& other_pos);
    std::vector<LinearConstraint> predFovRBConstraints(const std::vector<State>& pred_states,
                                                       const Vector& other_pos);
    std::vector<LinearConstraint> predRangeCBFConstraints(const std::vector<State>& pred_states,
                                                          const Vector& other_pos);
