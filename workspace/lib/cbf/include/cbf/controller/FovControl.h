...
class FovControl {
  public:
    ...
    FovControl(std::shared_ptr<FovCBF> cbf, int num_robots = 0, bool slack_mode = false,
               T slack_cost = 1000, T slack_decay_rate = 0.2);
    ~FovControl() = default;
    bool optimize(VectorDIM& cbf_u, const VectorDIM& desired_u, const State& current_state,
                  const std::vector<VectorDIM>& other_robot_positions,
                  const std::vector<Matrix>& other_robot_covs, const VectorDIM& u_min,
