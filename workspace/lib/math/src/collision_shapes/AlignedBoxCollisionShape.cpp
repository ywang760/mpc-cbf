...
AlignedBoxCollisionShape<T, DIM>::AlignedBoxCollisionShape(
    const typename AlignedBoxCollisionShape<T, DIM>::AlignedBox& collision_box_at_zero)
...
AlignedBoxCollisionShape<T, DIM>::convexHullPoints(
    const typename AlignedBoxCollisionShape<T, DIM>::VectorDIM& position) const {
...
template <typename T, unsigned int DIM>
typename AlignedBoxCollisionShape<T, DIM>::AlignedBox AlignedBoxCollisionShape<T, DIM>::boundingBox(
    const typename AlignedBoxCollisionShape<T, DIM>::VectorDIM& position) const {
...
AlignedBoxCollisionShape<T, DIM>::inflate(T inflation_amount) const {
    ...
    return std::make_shared<AlignedBoxCollisionShape<T, DIM>>(inflated_box_at_zero);
}
