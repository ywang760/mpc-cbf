...
class AlignedBoxCollisionShape : public CollisionShape<T, DIM> {
  public:
    ...
    virtual std::vector<VectorDIM> convexHullPoints(const VectorDIM& position) const override;
    ...
    /**
         ...
         */
    virtual std::shared_ptr<CollisionShape<T, DIM>> inflate(T inflation_amount) const override;

    bool operator==(const AlignedBoxCollisionShape& other) const {
        return collision_box_at_zero_.min() == other.collision_box_at_zero_.min() &&
               collision_box_at_zero_.max() == other.collision_box_at_zero_.max();
    }
