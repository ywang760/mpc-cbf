...
/**
     ...
     */
inline void
initializeLoggingFromEnv(spdlog::level::level_enum default_level = spdlog::level::info) {
...
initializeLogging(const std::string& pattern = "[%H:%M:%S] [%^%l%$] [%s:%# %!] %v",
...
std::string toString(const Eigen::MatrixBase<Derived>& M, int precision = 6) {
...
/*----- GiNaC matrix ---------------------------------------------------*/
inline std::string toString(const GiNaC::matrix& M, int precision = 6) {
    std::ostringstream ss;
    ss << "(" << M.rows() << "×" << M.cols() << ")\n";
    ss << std::fixed << std::setprecision(precision);
...
template <typename Derived>
inline void logMatrix(std::string_view label, const Eigen::MatrixBase<Derived>& M,
                      spdlog::level::level_enum lvl = spdlog::level::info, int precision = 6) {
    auto msg = fmt::format("{} {}", label, detail::toString(M, precision));
    spdlog::log(spdlog::source_loc{__FILE__, __LINE__, SPDLOG_FUNCTION}, lvl, "{}", msg);
}

inline void logMatrix(std::string_view label, const GiNaC::matrix& M,
                      spdlog::level::level_enum lvl = spdlog::level::info, int precision = 6) {
    auto msg = fmt::format("{} {}", label, detail::toString(M, precision));
    spdlog::log(spdlog::source_loc{__FILE__, __LINE__, SPDLOG_FUNCTION}, lvl, "{}", msg);
}
...
template <typename Derived>
inline void logVector(std::string_view label, const Eigen::MatrixBase<Derived>& v,
                      spdlog::level::level_enum lvl = spdlog::level::info, int precision = 6) {
