...
class SingleParameterCurveQPOperations {
  public:
    ...
    generateCurveFromSolution(const Vector& decision_variables) const = 0;
    ...
    virtual DecisionVariableBounds boundingBoxConstraint(const AlignedBox& bounding_box) const = 0;
    ...
    boundingBoxConstraintAll(const AlignedBox& bounding_box, uint64_t derivative_degree) const = 0;
