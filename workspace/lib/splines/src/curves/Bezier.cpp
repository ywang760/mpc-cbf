...
template <typename T, unsigned int DIM>
typename Bezier<T, DIM>::VectorDIM Bezier<T, DIM>::eval(T parameter,
                                                        uint64_t derivative_degree) const {
    using Row = math::Row<T>;
    // construct the bernsteinBasis
    Row basis_functions =
        bernsteinBasis(control_points_.size() - 1, max_parameter_, parameter, derivative_degree);

    VectorDIM result = VectorDIM::Zero();
    for (std::size_t control_point_idx = 0; control_point_idx < control_points_.size();
         ++control_point_idx) {
...
template class Bezier<double, 2U>;

template class Bezier<float, 2U>;

template class Bezier<double, 3U>;

template class Bezier<float, 3U>;
