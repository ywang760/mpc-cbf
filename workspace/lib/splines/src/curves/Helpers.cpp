...
nlohmann::json splines::convertPiecewiseCurveToJson(
    ...
    for (size_t piece_idx = 0; piece_idx < piecewise_curve.numPieces(); ++piece_idx) {
        ...
        for (size_t control_pt_index = 0; control_pt_index < bezier_piece.numControlPoints();
             ++control_pt_index) {
...
template <typename T, unsigned int DIM>
void splines::writePiecewiseCurveToJson(
    splines::SingleParameterPiecewiseCurve<T, DIM>& piecewise_curve, std::string filename) {
    using json = nlohmann::json;
...
template <typename T, unsigned int DIM>
void splines::writePiecewiseCurvesToJson(
...
template void splines::writePiecewiseCurveToJson<double, 3U>(
    splines::SingleParameterPiecewiseCurve<double, 3U>& piecewise_curve, std::string filename);
template void splines::writePiecewiseCurveToJson<float, 3U>(
    splines::SingleParameterPiecewiseCurve<float, 3U>& piecewise_curve, std::string filename);
template void splines::writePiecewiseCurveToJson<double, 2U>(
    splines::SingleParameterPiecewiseCurve<double, 2U>& piecewise_curve, std::string filename);
template void splines::writePiecewiseCurveToJson<float, 2U>(
    splines::SingleParameterPiecewiseCurve<float, 2U>& piecewise_curve, std::string filename);
