...
math::Row<T> bernsteinBasis(uint64_t bezier_degree, T max_parameter, T parameter,
    ...
    for (uint64_t i = 0; i <= bezier_degree; i++) {
        T base = 0.0;
        T mult = 1.0;
        for (uint64_t j = 0; j + derivative_degree <= bezier_degree; j++, mult *= parameter) {
            if (j + derivative_degree >= i) {
                const uint64_t comb_result =
                    math::comb(bezier_degree - i, j + derivative_degree - i);

                const uint64_t perm_result = math::perm(j + derivative_degree, derivative_degree);

                const T pow_result = math::pow(oneOverA, j + derivative_degree);

                base += (comb_result) * (pow_result) * (perm_result) *mult *
                        ((j + derivative_degree - i) % 2 == 0 ? 1 : -1);
            }
        }
        const uint64_t comb_result = math::comb(bezier_degree, i);
...
math::Matrix<T> bernsteinCoefficientMatrix(uint64_t bezier_degree, T max_parameter,
    ...
    if (bezier_degree == std::numeric_limits<uint64_t>::max()) {
        throw std::runtime_error(
            "bernsteinCoefficientMatrix: bezier_degree maxed, number of control points overflow.");
    }
    ...
    for (uint64_t i = 0; i < bezier_degree + 1; ++i) {
        ...
        T one_over_a_pow_j = math::pow<T>(oneOverA, i);

        for (uint64_t j = i; j < bezier_degree + 1; ++j, min1 *= -1, one_over_a_pow_j *= oneOverA) {
            bernstein_matrix(i, j) = dcombi * dminicombjmini * min1 * one_over_a_pow_j;

            if (bezier_degree != j &&
                dminicombjmini > std::numeric_limits<uint64_t>::max() / (bezier_degree - j)) {
        ...
        if ((bezier_degree != i) &&
            dcombi > std::numeric_limits<uint64_t>::max() / (bezier_degree - i)) {
    ...
    uint64_t jpermk = math::fac(derivative_degree);
...
template math::Row<double> bernsteinBasis(uint64_t bezier_degree, double max_parameter,
                                          double parameter, uint64_t derivative_degree);

template math::Row<float> bernsteinBasis(uint64_t bezier_degree, float max_parameter,
                                         float parameter, uint64_t derivative_degree);

template math::Matrix<double> bernsteinCoefficientMatrix(uint64_t bezier_degree,
                                                         double max_parameter,
                                                         uint64_t derivative_degree);

template math::Matrix<float> bernsteinCoefficientMatrix(uint64_t bezier_degree, float max_parameter,
                                                        uint64_t derivative_degree);
