...
BezierQPOperations<T, DIM>::BezierQPOperations(Params& p)
    : num_control_points_(p.num_control_points_), max_parameter_(p.max_parameter_) {}
...
BezierQPOperations<T, DIM>::generateCurveFromSolution(const Vector& decision_variables) const {
    ...
    if (decision_variables.rows() != numDecisionVariables()) {
        throw std::runtime_error(
            "number of decision variables does not match the required number.");
    }
    // create bezier curve
    std::unique_ptr<Bezier> bezier_ptr =
        std::make_unique<Bezier>(max_parameter_, std::vector<VectorDIM>());

    // append the control points from decision variable
    for (std::size_t control_point_idx = 0; control_point_idx < num_control_points_;
         ++control_point_idx) {
...
BezierQPOperations<T, DIM>::evalBasisRow(unsigned int dimension, T parameter,
    ...
    Row basis =
        bernsteinBasis(num_control_points_ - 1, max_parameter_, parameter, derivative_degree);
...
BezierQPOperations<T, DIM>::integratedSquaredDerivativeCost(uint64_t derivative_degree,
                                                            T lambda) const {
    ...
    if (derivative_degree <= /*degree=*/num_control_points_ - 1) {
        const Matrix bernstein_coefficient_matrix = bernsteinCoefficientMatrix(
            /*bezier_degree=*/num_control_points_ - 1, max_parameter_, derivative_degree);
        ...
        for (std::size_t i = 0; i < num_control_points_; i++) {
            for (std::size_t j = 0; j < num_control_points_; j++) {
                const T pow = math::pow<T>(max_parameter_, i + j + 1);
        ...
        Matrix cost =
            lambda * bernstein_coefficient_matrix * SQI * bernstein_coefficient_matrix.transpose();

        for (unsigned int dimension = 0; dimension < DIM; ++dimension) {
            quadratic_term.block(dimension * num_control_points_, dimension * num_control_points_,
                                 num_control_points_, num_control_points_) = cost;
        }
    }
...
BezierQPOperations<T, DIM>::evalCost(T parameter, uint64_t derivative_degree,
    ...
    const Matrix quadratic_term_per_dimension = lambda * basis.transpose() * basis;
    const Vector linear_term_per_dimension_except_target = -2 * lambda * basis.transpose();
    ...
    return CostAddition(quadratic_term, linear_term, constant_except_lambda * lambda);
}
...
BezierQPOperations<T, DIM>::evalConstraint(T parameter, uint64_t derivative_degree,
    ...
    const Row& basis = bernsteinBasis(/*bezier_degree=*/num_control_points_ - 1, max_parameter_,
    ...
    for (unsigned int dimension = 0; dimension < DIM; dimension++) {
        ...
        linear_constraints.push_back(
            LinearConstraint(coefficients, target(dimension), target(dimension)));
    }
    return linear_constraints;
}
...
template <typename T, unsigned int DIM>
std::vector<typename BezierQPOperations<T, DIM>::LinearConstraint>
BezierQPOperations<T, DIM>::evalBound(T parameter, uint64_t derivative_degree, const VectorDIM& LB,
...
BezierQPOperations<T, DIM>::boundingBoxConstraint(const AlignedBox& bounding_box) const {
    ...
    for (unsigned int dimension_idx = 0; dimension_idx < DIM; ++dimension_idx) {
        ...
        for (std::size_t control_point_idx = 0; control_point_idx < num_control_points_;
             ++control_point_idx) {
            lower_bounds(dimension_idx * num_control_points_ + control_point_idx) =
                bounding_box_min_for_dimension;
            upper_bounds(dimension_idx * num_control_points_ + control_point_idx) =
                bounding_box_max_for_dimension;
        }
    }
...
template <typename T, unsigned int DIM>
std::vector<typename BezierQPOperations<T, DIM>::LinearConstraint>
BezierQPOperations<T, DIM>::boundingBoxConstraintAll(const AlignedBox& bounding_box,
                                                     uint64_t derivative_degree) const {
    ...
    const T perm = math::perm(num_control_points_ - 1, derivative_degree);
    ...
    for (uint64_t i = 0; i < num_control_points_ - derivative_degree; ++i) {
        ...
        for (unsigned int d = 0; d < DIM; ++d) {
            ...
            coefficients.block(0, d * num_control_points_, 1, num_control_points_) =
                control_point_multipliers;
            constraints.push_back(
                LinearConstraint(coefficients, bounding_box.min()(d), bounding_box.max()(d)));
        }
    }
...
template <typename T, unsigned int DIM>
std::vector<typename BezierQPOperations<T, DIM>::LinearConstraint>
BezierQPOperations<T, DIM>::hyperplaneConstraintAll(const Hyperplane& hyperplane, T epsilon) const {
    ...
    for (std::size_t control_point_idx = 0; control_point_idx < num_control_points_;
        ...
        coefficients.setZero();
        for (unsigned int dimension_idx = 0; dimension_idx < DIM; ++dimension_idx) {
            coefficients(dimension_idx * num_control_points_ + control_point_idx) =
                hyperplane.normal()(dimension_idx);
        }
        linear_constraints.push_back(LinearConstraint(
            coefficients, std::numeric_limits<T>::lowest(), -hyperplane.offset() - epsilon));
    }
    return linear_constraints;
}
...
template <typename T, unsigned int DIM>
std::vector<typename BezierQPOperations<T, DIM>::LinearConstraint>
BezierQPOperations<T, DIM>::hyperplaneConstraintAt(T parameter, const Hyperplane& hyperplane,
                                                   T epsilon) const {
    ...
    coefficients.setZero();
    for (unsigned int dimension_idx = 0; dimension_idx < DIM; ++dimension_idx) {
        coefficients.block(0, dimension_idx * num_control_points_, 1, num_control_points_) =
            basis * hyperplane.normal()(dimension_idx);
    }
    linear_constraints.push_back(LinearConstraint(coefficients, std::numeric_limits<T>::lowest(),
