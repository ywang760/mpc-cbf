...
void SingleParameterPiecewiseCurveQPGenerator<T, DIM>::addPiece(
    ...
    for (std::size_t decision_variable_idx = 0;
         decision_variable_idx < num_decision_variables_of_new_piece; ++decision_variable_idx) {
...
SingleParameterPiecewiseCurveQPGenerator<T, DIM>::generateCurveFromSolution() const {
    ...
    using SingleParameterCurve = typename SingleParameterCurveQPOperations::SingleParameterCurve;
    ...
    for (std::size_t piece_idx = 0; piece_idx < piece_operations_ptrs_.size(); ++piece_idx) {
        ...
        std::unique_ptr<SingleParameterCurve> curve =
            piece_operations_ptrs_.at(piece_idx)->generateCurveFromSolution(solution);
...
template <typename T, unsigned int DIM>
void SingleParameterPiecewiseCurveQPGenerator<T, DIM>::addIntegratedSquaredDerivativeCost(
    uint64_t derivative_degree, T lambda) {
    for (std::size_t piece_idx = 0; piece_idx < piece_operations_ptrs_.size(); ++piece_idx) {
        const CostAddition& cost_addition =
            piece_operations_ptrs_.at(piece_idx)->integratedSquaredDerivativeCost(derivative_degree,
                                                                                  lambda);
...
template <typename T, unsigned int DIM>
void SingleParameterPiecewiseCurveQPGenerator<T, DIM>::addEvalCost(T parameter,
    ...
    const CostAddition& cost_addition =
        piece_operations_ptrs_.at(piece_idx_and_parameter.piece_idx())
            ->evalCost(piece_idx_and_parameter.parameter(), derivative_degree, target, lambda);

    addCostAdditionForPiece(piece_idx_and_parameter.piece_idx(), cost_addition);
}

template <typename T, unsigned int DIM>
void SingleParameterPiecewiseCurveQPGenerator<T, DIM>::addEvalConstraint(T parameter,
                                                                         uint64_t derivative_degree,
                                                                         const VectorDIM& target) {
    const PieceIndexAndParameter& piece_idx_and_parameter = getPieceIndexAndParameter(parameter);
...
template <typename T, unsigned int DIM>
void SingleParameterPiecewiseCurveQPGenerator<T, DIM>::addCostAdditionForPiece(
    ...
    if (num_decision_variables != cost_addition.linear_term().rows() ||
        ...
        throw std::runtime_error(
            "SingleParameterPiecewiseCurveQPGenerator::addCostAdditionForPiece:"
...
template <typename T, unsigned int DIM>
void SingleParameterPiecewiseCurveQPGenerator<T, DIM>::addContinuityConstraint(
    std::size_t piece_idx, uint64_t derivative_degree) {
    using Row = math::Row<T>;
    // piece_idx out_of_range error
    if (piece_idx + 1 >= cumulative_max_parameters_.size()) {
        throw std::runtime_error(
            "SingleParameterPiecewiseCurveQPGenerator::addContinuityConstraint:"
            " piece index out of range. piece_idx: " +
    ...
    for (unsigned int dimension = 0; dimension < DIM; ++dimension) {
        const Row& first_piece_row = piece_operations_ptrs_.at(piece_idx)->evalBasisRow(
            /*dimension=*/dimension,
            /*parameter=*/piece_operations_ptrs_.at(piece_idx)->max_parameter(),
            /*derivative_degree=*/derivative_degree);
        const Row& second_piece_row = piece_operations_ptrs_.at(piece_idx + 1)
                                          ->evalBasisRow(
                                              /*dimension=*/dimension,
                                              /*parameter=*/0,
                                              /*derivative_degree=*/derivative_degree);
...
template <typename T, unsigned int DIM>
void SingleParameterPiecewiseCurveQPGenerator<T, DIM>::addBoundingBoxConstraint(
    const AlignedBox& bounding_box) {
    for (std::size_t piece_idx = 0; piece_idx < piece_operations_ptrs_.size(); ++piece_idx) {
        DecisionVariableBounds decision_variable_bounds =
            piece_operations_ptrs_.at(piece_idx)->boundingBoxConstraint(bounding_box);
        addDecisionVariableBoundsForPiece(piece_idx, decision_variable_bounds);
    }
}

template <typename T, unsigned int DIM>
void SingleParameterPiecewiseCurveQPGenerator<T, DIM>::addBoundingBoxConstraintAll(
    const AlignedBox& aligned_box, uint64_t derivative_degree) {
    for (std::size_t piece_idx = 0; piece_idx < piece_operations_ptrs_.size(); ++piece_idx) {
        const std::vector<LinearConstraint>& piece_constraints =
            piece_operations_ptrs_.at(piece_idx)->boundingBoxConstraintAll(aligned_box,
                                                                           derivative_degree);
...
template <typename T, unsigned int DIM>
void SingleParameterPiecewiseCurveQPGenerator<T, DIM>::addPositionConstraintForPiece(
    std::size_t piece_idx, const VectorDIM& position) {
    if (piece_idx >= cumulative_max_parameters_.size()) {
        throw std::invalid_argument("SingleParameterPiecewiseCurveQPGenerator::"
                                    ...
                                    std::to_string(cumulative_max_parameters_.size()));
    }
...
template <typename T, unsigned int DIM>
void SingleParameterPiecewiseCurveQPGenerator<T, DIM>::addHyperplaneConstraintForPiece(
...
void SingleParameterPiecewiseCurveQPGenerator<T, DIM>::addDecisionVariableBoundsForPiece(
    std::size_t piece_idx, const DecisionVariableBounds& decision_variable_bounds) {
    std::size_t num_decision_variables = variable_map_.at(piece_idx).size();
    ...
    for (std::size_t decision_variable_idx = 0; decision_variable_idx < num_decision_variables;
         ++decision_variable_idx) {
        qpcpp::Variable<T>* var_ptr = variable_map_.at(piece_idx).at(decision_variable_idx);
...
template <typename T, unsigned int DIM>
SingleParameterPiecewiseCurveQPGenerator<T, DIM>::PieceIndexAndParameter::PieceIndexAndParameter(
...
template <typename T, unsigned int DIM>
T SingleParameterPiecewiseCurveQPGenerator<T, DIM>::PieceIndexAndParameter::parameter() const {
    return parameter_;
}
