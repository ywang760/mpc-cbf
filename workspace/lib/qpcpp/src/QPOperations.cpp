...
template <typename T>
QPOperations<T>::CostAddition::CostAddition(const Matrix& quadratic_term, const Vector& linear_term,
                                            T constant)
    : quadratic_term_(quadratic_term), linear_term_(linear_term), constant_(constant) {}

template <typename T>
const typename QPOperations<T>::Matrix& QPOperations<T>::CostAddition::quadratic_term() const {
    return quadratic_term_;
}

template <typename T>
const typename QPOperations<T>::Vector& QPOperations<T>::CostAddition::linear_term() const {
    return linear_term_;
}
...
template <typename T>
QPOperations<T>::LinearConstraint::LinearConstraint(const Row& coefficients, T lower_bound,
                                                    T upper_bound)
    : coefficients_(coefficients), lower_bound_(lower_bound), upper_bound_(upper_bound) {}

template <typename T>
const typename QPOperations<T>::Row& QPOperations<T>::LinearConstraint::coefficients() const {
...
template <typename T>
QPOperations<T>::DecisionVariableBounds::DecisionVariableBounds(const Vector& lower_bounds,
