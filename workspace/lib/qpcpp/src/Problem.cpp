...
namespace qpcpp {
    ...
    template <typename T>
    LinearConstraint<T>::LinearConstraint(const Problem& context_problem, T min,
                                          T max)
            : context_problem_(context_problem), min_(min), max_(max) {}

    template <typename T>
    void LinearConstraint<T>::setCoefficient(const Variable* variable,
                                                     T coefficient) {
        if (!context_problem_.hasVariable(variable)) {
            throw std::runtime_error("LinearConstraint::setCoefficient: variable does not exist in the convex problem");
        }
    ...
    template <typename T>
    T LinearConstraint<T>::getCoefficient(
            const Variable* variable) const {
        if (!context_problem_.hasVariable(variable)) {
            throw std::runtime_error("LinearConstraint::getCoefficient: variable does not exist in the convex problem");
        }
    ...
    void CostFunction<T>::addQuadraticTerm(const Variable* first_variable,
        ...
        if (!context_problem_.hasVariable(first_variable)) {
            throw std::runtime_error("CostFunction::addQuadraticTerm: first_variable does not exist in the context problem");
        }

        if (!context_problem_.hasVariable(second_variable)) {
            throw std::runtime_error("CostFunction::addQuadraticTerm: second_variable does not exist in the context problem");
        }
        ...
        if (quadratic_coefficients_.find(first_variable) ==
            quadratic_coefficients_.end()) {
            quadratic_coefficients_.emplace(
                    first_variable, std::unordered_map<const Variable*, T>());
        }

        std::unordered_map<const Variable*, T>& first_variable_coefficients =
                quadratic_coefficients_.at(first_variable);

        if (first_variable_coefficients.find(second_variable) ==
            first_variable_coefficients.end()) {
    ...
    template <typename T>
    T CostFunction<T>::getQuadraticCoefficient(
            const Variable* first_variable, const Variable* second_variable) const {
        if (!context_problem_.hasVariable(first_variable)) {
            throw std::runtime_error("CostFunction::getQuadraticCoefficient: first_variable does not exist in the context problem");
        }

        if (!context_problem_.hasVariable(second_variable)) {
            throw std::runtime_error("CostFunction::getQuadraticCoefficient: second_variable does not exist in the context problem");
        }
        ...
        if (typename std::unordered_map<const Variable*,
                    std::unordered_map<const Variable*, T>>::
            const_iterator first_variable_coefficients_it =
                    quadratic_coefficients_.find(first_variable);
                first_variable_coefficients_it != quadratic_coefficients_.end()) {
            const std::unordered_map<const Variable*, T>&
                    first_variable_coefficients =
                    first_variable_coefficients_it->second;

            if (typename std::unordered_map<const Variable*, T>::const_iterator
                        coefficient_it =
    ...
    template <typename T>
    void CostFunction<T>::addLinearTerm(const Variable* variable,
                                        T coefficient) {
        if (!context_problem_.hasVariable(variable)) {
            throw std::runtime_error("CostFunction::addLinearTerm: variable does not exist in the context problem");
        }
    ...
    T CostFunction<T>::getLinearCoefficient(const Variable* variable) const {
        if (!context_problem_.hasVariable(variable)) {
            throw std::runtime_error("CostFunction::getLinearCoefficient: variable does not exist in the context problem");
        }
    ...
    template <typename T>
    Problem<T>::Problem()
            : num_variables_(0), num_linear_constraints_(0), cost_function_(*this) {}
    ...
    template <typename T>
    const std::forward_list<LinearConstraint<T>>& Problem<T>::linear_constraints()
    const {
