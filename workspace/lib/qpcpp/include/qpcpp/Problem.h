...
class CostFunction {
  public:
    ...
    // context problem
    void addQuadraticTerm(const Variable* first_variable, const Variable* second_variable,
                          T coefficient);
    ...
    T getQuadraticCoefficient(const Variable* first_variable,
...
class Problem {
  public:
    ...
    LinearConstraint* addLinearConstraint(T min = std::numeric_limits<T>::lowest(),
                                          T max = std::numeric_limits<T>::max());
