...
bool BezierMPC<T, DIM>::optimize(SingleParameterPiecewiseCurve& result_curve,
    ...
    for (size_t i = 0; i < other_robot_positions.size(); ++i) {
        ...
        qp_generator_.addHyperplaneConstraintForPiece(
            0, shifted_hyperplane); // add the collision avoidance constraint on the first segment
    }
...
BezierMPC<T, DIM>::generatorDerivativeControlInputs(uint64_t derivative_degree) {
    Matrix U_basis = qp_generator_.piecewise_operations_ptr()->evalSamplingBasisMatrix(
        ts_samples_, derivative_degree); // [3I, num_piece*dim*num_control_pts]
    Vector control_pts_variables_value =
        qp_generator_.getVariablesValue(); // [num_piece*dim*num_control_pts, 1]
