...
namespace qpcpp {
    ...
    template <typename T>
    CPLEXSolver<T>::CPLEXSolver(T optimality_tolerance, T feasibility_tolerance,
                                T time_limit)
    ...
    template <typename T>
    void CPLEXSolver<T>::setSolution(
            const IloEnv& cplex_env, const IloCplex& cplex_solver,
        ...
        for (std::size_t variable_idx = 0;
             variable_idx < cplex_indices_problem_variables.size();
             ++variable_idx) {
    ...
    SolveStatus CPLEXSolver<T>::solve(Problem& problem) {
        ...
        // solution is found
        const typename std::forward_list<LinearConstraint>&
                problem_linear_constraints = problem.linear_constraints();
        ...
        typename std::forward_list<Variable>::iterator problem_variable_it =
                problem_variables.begin();
        for (std::size_t variable_idx = 0; variable_idx < problem.numVariables();
            ...
            IloNumVar cplex_variable(cplex_env, problem_variable.min(),
                                     problem_variable.max(), ILOFLOAT);
        ...
        for (const LinearConstraint& problem_linear_constraint :
                problem_linear_constraints) {
            IloExpr cplex_expr(cplex_env);
            for (std::size_t variable_idx = 0;
                 variable_idx < problem.numVariables(); ++variable_idx) {
                const Variable* problem_variable_ptr =
                        cplex_indices_problem_variables[variable_idx];
                T coefficient =
                        problem_linear_constraint.getCoefficient(problem_variable_ptr);
                cplex_expr +=
                        cplex_variables[variable_idx] * (coefficient);
            }

            IloRange cplex_range(cplex_env, problem_linear_constraint.min(),
                                 cplex_expr, problem_linear_constraint.max());
        ...
        for (std::size_t first_variable_idx = 0;
             first_variable_idx < problem.numVariables(); ++first_variable_idx) {
            const Variable* first_variable_ptr =
                    cplex_indices_problem_variables[first_variable_idx];

            T coefficient =
                    cost_function->getLinearCoefficient(first_variable_ptr);
            cplex_linear_cost +=
                    cplex_variables[first_variable_idx] * (coefficient);

            for (std::size_t second_variable_idx = first_variable_idx;
                 second_variable_idx < problem.numVariables();
                 ++second_variable_idx) {
                const Variable* second_variable_ptr =
                        cplex_indices_problem_variables[second_variable_idx];
                T coefficient =
                        cost_function->getQuadraticCoefficient(first_variable_ptr,
                                                               second_variable_ptr);

                cplex_quadratic_cost += cplex_variables[first_variable_idx] *
                                        cplex_variables[second_variable_idx] *
                                        (coefficient);
            }
        }
        ...
        IloObjective cplex_objective(
                cplex_env,
                cplex_quadratic_cost + cplex_linear_cost + cost_function->constant(),
        ...
        cplex_solver.setParam(IloCplex::Param::OptimalityTarget,
        ...
        try {
            ...
            cplex_solver.setParam(IloCplex::Param::RootAlgorithm,
                                  IloCplex::AutoAlg);
            cplex_solver.solve();
        ...
        switch (cplex_solve_status) {
            ...
            case IloAlgorithm::Status::Feasible: {
                setSolution(cplex_env, cplex_solver, cplex_variables,
                            cplex_indices_problem_variables);
