...
namespace splines {
    ...
    void SingleParameterPiecewiseCurve<T, DIM>::addPiece(std::unique_ptr<SingleParameterCurve> &&piece_ptr) {
    ...
    SingleParameterPiecewiseCurve<T, DIM>::getPiece(std::size_t piece_idx) const {
        if (piece_idx >= piece_ptrs_.size()) {
            throw std::out_of_range("SingleParameterPiecewiseCurve::getPiece: piece index out of range.");
        }
    ...
    template<typename T, unsigned int DIM>
    void SingleParameterPiecewiseCurve<T, DIM>::setMaxParameter(std::size_t piece_idx, T new_max_parameter) {
        if (piece_idx >= piece_ptrs_.size()) {
            throw std::out_of_range("SingleParameterPiecewiseCurve::setMaxParameter: piece index out of range.");
        }

        piece_ptrs_[piece_idx]->set_max_parameter(new_max_parameter);

        for (std::size_t idx = piece_idx; idx < cumulative_max_parameters_.size();
             ++idx) {
            if (idx == 0) {
                cumulative_max_parameters_[idx] = piece_ptrs_[idx]->max_parameter();
            } else {
                cumulative_max_parameters_[idx] =
                        cumulative_max_parameters_[idx - 1] +
                        piece_ptrs_[idx]->max_parameter();
            }
        }
    }
    ...
    void SingleParameterPiecewiseCurve<T, DIM>::scaleMaxParameters(T scaling_factor) {
        ...
        if (scaling_factor < 0) {
            throw std::invalid_argument(
                    "SingleParameterPiecewiseCurve::scaleMaxParameters: scaling_factor is negative. scaling_factor: " +
        ...
        for (std::size_t piece_idx = 0; piece_idx < numPieces(); ++piece_idx) {
            if (piece_idx == 0) {
                cumulative_max_parameters_[piece_idx] =
                        piece_ptrs_[piece_idx]->max_parameter();
            } else {
                cumulative_max_parameters_[piece_idx] =
                        cumulative_max_parameters_[piece_idx - 1] +
                        piece_ptrs_[piece_idx]->max_parameter();
            }
        }
    }
    ...
    SingleParameterPiecewiseCurve<T, DIM>::eval(T parameter, uint64_t derivative_degree) const {
        ...
        if (parameter < 0 || parameter > max_param) {
            throw std::invalid_argument("SingleParameterPiecewiseCurve::eval: parameter is out of range.");
        }
        ...
        if (cumulative_parameter_iterator == cumulative_max_parameters_.end()) {
            throw std::runtime_error("SingleParameterPiecewiseCurve::eval: std::lower_bound failed, but "
        ...
        if (piece_idx == 0) {
            ...
            return piece_ptrs_[piece_idx]->eval(
                    std::min(piece_max_parameter,
                             parameter - cumulative_max_parameters_[piece_idx - 1]),
                    derivative_degree);
        }
    }
    ...
    SingleParameterPiecewiseCurve<T, DIM>::maximumDerivativeMagnitude(uint64_t derivative_degree) const {
        ...
        for (std::size_t piece_idx = 0; piece_idx < piece_ptrs_.size(); ++piece_idx) {
            ...
            const MaximumDerivativeMagnitude &magnitude_parameter_pair = piece_ptrs_[piece_idx]->maximumDerivativeMagnitude(
                    derivative_degree);
        ...
        return MaximumDerivativeMagnitude{
                .magnitude = maximum_derivative_magnitude,
                .parameter = maximum_derivative_magnitude_parameter};
    }

    template
    class SingleParameterPiecewiseCurve<double, 3U>;

    template
    class SingleParameterPiecewiseCurve<double, 2U>;

    template
    class SingleParameterPiecewiseCurve<float, 3U>;

    template
    class SingleParameterPiecewiseCurve<float, 2U>;
