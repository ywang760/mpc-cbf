...
{   
    ...
    void ParticleFilter::init(int particles_num, Eigen::VectorXd initState, Eigen::MatrixXd initCov, Eigen::MatrixXd processCov, Eigen::MatrixXd measurementCov)
    {
        ...
        {
            throw std::runtime_error("ParticleFilter Error: state size does not match state covariance size!");
        }
        if (state_size_ != Q_.cols() || state_size_ != Q_.rows())
        {
            throw std::runtime_error("ParticleFilter Error: state size does not match particles distribution size!");
        }
    ...
    void ParticleFilter::update(Eigen::VectorXd measurement, Eigen::MatrixXd cov)
    {
        ...
        {
            throw std::runtime_error("ParticleFilter Error: state size does not match measurement size!");
        }
        if (state_size_ != cov.rows() || state_size_ != cov.cols())
        {
            throw std::runtime_error("ParticleFilter Error: state size does not match measurement covariance size!");
        }
    ...
    void ParticleFilter::setWeights(Eigen::VectorXd weights)
    {
        if (weights.size() != n_)
        {
            throw std::runtime_error("ParticleFilter Error: Weights size different from number of particles!");
        }
        w_ = weights;
    }
