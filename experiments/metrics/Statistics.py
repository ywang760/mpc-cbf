...
for i in range(len(experiment_key)):
    ...
    for r_idx, num_r in enumerate(num_robot):
        ...
        for exp_idx in range(num_exp):
            ...
            print("success: ", success, "makespan: ", makespan)
            success_rate_dict[key][r_idx].append(float(success))
            makespan_dict[key][r_idx].append(makespan)
...
for key in experiment_key:
    ...
    makespan_ci_arr_list.append(makespan_ci_dict[key]["ci"])
...
histogram_list_CI_plot(num_robot_arr, makespan_samples, makespan_mean_arr_list, makespan_ci_arr_list, makespan_label_list, colors, xlabel="Number of Robots", ylabel="Makespan", legend=False, save_name="./hist_makespan_ci.pdf")